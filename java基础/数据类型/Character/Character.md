...
![image-20210709180551382](https://raw.githubusercontent.com/bitchigo/msImg/main/20210709180551.png)
...
```java
   public static UnicodeScript of(int codePoint) {
            if (!isValidCodePoint(codePoint)) //是否为有效的码值
                throw new IllegalArgumentException(
                    String.format("Not a valid Unicode code point: 0x%X", codePoint));
            int type = getType(codePoint);
            // leave SURROGATE and PRIVATE_USE for table lookup
            if (type == UNASSIGNED) //如果为无效值
                return UNKNOWN;
            int index = Arrays.binarySearch(scriptStarts, codePoint);  //去表查找  二分查找
       		//不存在则会返回-的数值+1
            if (index < 0)  //说明不存在的情况
                index = -index - 2; //修正数值
...
> 我们利用Unicode编码是否在 0x4E00–0x9FCC 来判断某字符是否为汉字，就是因为，有个UnicodeBlock 专门划分为存储汉字 (准确的说是 CJK统一汉字)，这个UnicodeBlock叫做 CJK Unified Ideographs，总共定义了 74,617 个汉字。
